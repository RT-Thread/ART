...
public class STM32DfuUploader extends Uploader {
	...
	public boolean uploadUsingPreferences(String buildPath, String className,
			boolean usingProgrammer) throws RunnerException, SerialException {
		this.verbose = verbose;
		Map<String, String> boardPreferences = Base.getBoardPreferences();

		if ("ip".equals(boardPreferences.get("upload.protocol"))) {
			// download the program by ART IP
			return uploadViaIP(buildPath, className);
		}
		...
		if (usingProgrammer || boardPreferences.get("upload.protocol") == null) {
			...
			if (programmer.indexOf(":") != -1) {
				target = Base.targetsTable.get(programmer.substring(0,
						programmer.indexOf(":")));
	...
	private boolean uploadViaIP(String buildPath, String className)	throws RunnerException {
		String app;
		String addr;
		int offset;
		Map<String, String> boardPreferences = Base.getBoardPreferences();
		...
		app = buildPath + File.separator + className + ".mo";
		addr = boardPreferences.get("upload.addr");
	...
	protected boolean burnRomFS(Collection params) throws RunnerException {
		...
		params.add("-D");
		params.add(Base.getTarget().getFolder().getAbsolutePath()
				+ File.separator + "root.bin");
	...
	public boolean buildRomFS() {
		...
		if (Base.isLinux()) {
			...
			mkromfsProgramer = Base.getHardwarePath() + "\\tools\\bin\\"
					+ "mkromfs";
		}
		...
		List commandBuildRomFS = new ArrayList(Arrays.asList(new String[] {
				...
				"--addr",
				"0x08080000",
				Base.getTarget().getFolder().getAbsolutePath() + File.separator
						+ "root" }));
	...
	public boolean art(Collection params) throws RunnerException {
		List commandDownloader = new ArrayList();

		/* get dfu-util command */
		if (Base.isLinux()) {
			if ((new File(Base.getHardwarePath() + "/tools/bin/art.py")).exists()) {
				commandDownloader.add(Base.getHardwarePath() + "/tools/bin/art.py");
			} else {
				commandDownloader.add("art.py");
			}
		} else {
			commandDownloader.add(Base.getHardwarePath() + "\\tools\\bin\\art");
		}
	...
	public boolean dfu(Collection params) throws RunnerException {
		...
		/* get dfu-util command */
		if (Base.isLinux()) {
			if ((new File(Base.getHardwarePath() + "/tools/" + "stm32-dfu"))
					.exists()) {
				commandDownloader.add(Base.getHardwarePath()
						+ "/tools/stm32-dfu/dfu-util");
			} else {
				commandDownloader.add("dfu-util");
			}
		} else {
			commandDownloader.add(Base.getHardwarePath()
					+ "\\tools\\stm32-dfu\\dfu-util");
		}
	...
	protected boolean executeCommand(Collection commandDownloader)
